...
### Рішення дизайну: обсяг MVP (Phase 5)

Без окремої сутності `Task`/task tracker (картки, дедлайни, виконавці, статуси задач) у MVP. `Project.status` (planning/active/completed/archived) — єдиний механізм відстеження прогресу проєкту в цій фазі.

Обґрунтування: VISION.md описує "проєкти" як спільну роботу над особистим розвитком (книжковий клуб, ініціатива), не софт-менеджмент; Principle 3 ("Якість > Кількість") і Principle 6 ("Розвиток") обидва проти додавання ваги, яка не відповідає на "як це допомагає рости". Повноцінний task tracker — природне розширення пізніших фаз, не MVP; жоден документ (ARCHITECTURE.md, DATABASE.md) не проєктував `Task`-сутність заздалегідь.
...
`TODO`: внутрішня структура (лінійний список кроків/уроків чи щось складніше) не визначена. Концепція "Навчальних кімнат" (нижче) тепер ухвалена, але це не сама структура LearningPath — розширення цієї таблиці лишається окремою задачею, коли буде продуктове рішення саме про кроки/уроки.
...
Звʼязки: N:1 User (host); N:1 LearningPath (optional); N:N User через LearningRoomMember.

### LearningRoomMember (нова)

| Поле      | Тип                      | Опис |
| --------- | ------------------------ | ---- |
| id        | uuid (PK)                |      |
| room_id   | uuid (FK → LearningRoom) |      |
| user_id   | uuid (FK → User)         |      |
| joined_at | timestamp                |      |

Без поля `role` — пласке членство (рішення дизайну нижче).

### Рішення дизайну

- **Постійний простір, не сесія**: на відміну від Event (starts_at/ends_at), кімната не має завершення — це радше персистентна група навколо теми, ближче до Project/Community за формою.
- **Публічна й відкрита**: на відміну від Project (видимість лише учасникам), список кімнат — публічний каталог; будь-хто авторизований може знайти й приєднатися. Узгоджено з формулюванням DEVELOPMENT_PLAN.md ("UI: список кімнат", без "користувача" — на відміну від "UI: список проєктів **користувача**") і з VISION.md ("знаходять однодумців").
- **Один хост, без багаторівневих ролей**: творець кімнати — хост (той самий підхід, що й `Event.organizerId`), решта учасників рівноправні. Простіше за Project (admin/member) — немає продуктової підстави для складнішої моделі прав у навчальній групі.
- **Опційний звʼязок з LearningPath**: кімната може групуватись навколо конкретного курсу (`learning_path_id`, nullable), але це не обовʼязково — кімната може існувати і без прив'язки до курсу.
- **Матеріали — окрема сутність `Material`**, не текстове поле: список посилань/файлів із власним CRUD, а не одне вільне текстове поле в самій кімнаті (свідомо ширший обсяг, ніж мінімалістичне рішення "без task tracker" для Project — продуктове рішення, не архітектурне обмеження). Через це в DEVELOPMENT_PLAN.md додано окремі задачі на ендпоінти й UI матеріалів, яких спочатку в плані не було.
- **Технологія real-time для спільної роботи** (шеринг матеріалів/whiteboard) — **жодної не потрібно для MVP**. Обґрунтування: жодна інша списочна функція в застосунку (Goals, Events, Projects, Diary, тепер і Material) не має live push — оновлення відбувається через звичайний refetch після власної дії користувача, той самий підхід тут. Існуючий WS-сервіс (`server/ws-server.ts`, ARCHITECTURE.md → Tech Stack) явно призначений лише для Messaging/Notifications/майбутнього Voice-Video — навчальні кімнати там ніколи не згадувались. Live-whiteboard (canvas/CRDT) — окремий порядок складності, непропорційний для MVP-списку матеріалів; відкладається на майбутню фазу, якщо колись стане продуктовою вимогою.
...
| title      | string, not null         |                                             |
| url        | string, not null         | матеріал завжди посилання/файл, не nullable |
| note       | text, nullable           | опційний контекст/опис                      |
| created_at | timestamp                |                                             |
| updated_at | timestamp                |                                             |

Звʼязки: N:1 LearningRoom; N:1 User (added_by). Явний FK на кімнату, не поліморфний звʼязок — той самий підхід, що й Bookmark/Reaction.

**Права** (для задачі "CRUD-ендпоінти матеріалів"): додавати може будь-який учасник кімнати (crowdsourced список ресурсів, узгоджено з VISION.md — "діляться досвідом"), не лише хост; редагувати/видаляти — лише автор запису або хост кімнати (модерація).
