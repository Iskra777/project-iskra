...
**Не редагується тут:** `email`, `username` (зміна пошти вимагала б повторної верифікації, зміна username зачіпає унікальність/URL — жодне не запитувалось), `avatarUrl` (окрема задача — сховище файлів).
