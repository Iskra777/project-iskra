...
`POST /api/conversations/:id/messages` (вище) шле `pg_notify('new_message', {conversationId, messageId})` **усередині тієї ж транзакції**, що й запис повідомлення — Postgres доставляє нотифікацію лише після коміту, тобто rollback нічого не пошле. WS-сервер підписаний (`LISTEN new_message`), за нотифікацією довантажує повне повідомлення через Prisma й розсилає його сокетам, які приєднані до цієї розмови. API-роут і WS-процес більше нічого одне про одного не знають.
...
Індикатор "прочитано"/"набирає текст", UI inbox і UI чату, що споживають цей протокол.
