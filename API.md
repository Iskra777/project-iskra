...
`title` — заповнений лише для `type: "group"` (DATABASE.md#conversation), інакше `null`. `otherParticipant` — обчислений лише для `type: "direct"` (інший з двох `participants`), `null` для `group` — там немає єдиного "іншого", дивитись повний `participants[]`. `lastMessage` — `null`, якщо в розмові ще нема повідомлень. `unread` = останнє повідомлення надіслав не я, і воно новіше за мій `ConversationParticipant.lastReadAt` (той самий механізм, що й `PATCH .../read`).

`TODO`: `participants[]` уже повертається для будь-якого типу розмови, але створити `group`-розмову поки що нема як — UI/ендпоінти створення групи й керування учасниками ще не реалізовані (DEVELOPMENT_PLAN.md → "Групові чати").
