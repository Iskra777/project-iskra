...
#0B0F19 (веб). На мобільному — `#0A0A0C` (`apps/mobile/constants/Colors.ts`): той самий відтінок читався помітно синім, до того ж `View` з `components/Themed.tsx` завжди малює цей колір непрозоро за замовчуванням (на відміну від вебу, де незаданий CSS-фон прозорий) — тож будь-який вкладений View без явного `transparent` лягав синім шаром поверх карток. За проханням власника продукту нейтралізовано, лише для apps/mobile.

Cards

#141A27 (веб). На мобільному — `#1A1A1F`, та сама причина.
...
На `lg:` (1024px+) до нижньої панелі додається `components/sidebar-nav.tsx` — фіксована бічна панель зліва (`w-56`), той самий список пунктів (спільний `lib/nav-items.ts`, щоб не дублювати між нижньою й бічною панеллю), той самий glow на активному пункті, міні-картка користувача (аватар + username) знизу панелі. Нижня панель ховається (`lg:hidden`) — на десктопі її замінює бічна. Верхня шапка (`components/nav.tsx`) зсувається `lg:pl-56`, щоб лого/вихід не опинялись під панеллю. Профіль і решта сторінок не змінені — лише розташування навігації.
